...
import {ActivityIndicator, Keyboard, KeyboardAvoidingView, StyleSheet, Alert, TextInput, Dimensions, TouchableOpacity} from 'react-native';
...
export default ({navigation})=>{
  ...
  const handleAuth = useCallback(async type => {
	...
	const renderLogin = () => {
		...
		<Block center  middle flex={0.7}>
			...
			<Block row flex={false}> 
				<Button transparent onPress={()=> navigation.push('Forgot')} style={{position : 'relative', right : -width * 0.25, top : -24}}> 
					<Text white caption style={{textDecorationLine : 'underline'}}>Forgot Password </Text>
				</Button>
			</Block>
			<Block  style={styles.inputContainer}>
						<Button  onPress={()=> handleAuth()}>
							{
									loading ? (<ActivityIndicator size="small" color="white" />) : ( <Text center bold white>Sign In</Text> )
							}
						</Button>
			</Block>
			<Block row center flex={false}>
						<Block style={styles.divider}/>
						<Text caption white>or</Text>
						<Block style={styles.divider}/>
			</Block>
		</Block>
		);
	...
	const renderSocials = () => (
		<Block style ={{ flexDirection : 'row', flex : 0,  justifyContent: 'center', alignItems: 'center'}}> 
				<TouchableOpacity
					activeOpacity={0.8}
		...
		<KeyboardAvoidingView behavior="padding"  style={{flex : 1, justifyContent : 'center', backgroundColor : 'white'}} keyboardVerticalOffset = {-28}>
					<LinearGradient
						...
						<Block bottom center flex={false} style={{marginBottom : 16, paddingRight : 16, paddingLeft : 16}}>
							<Button onPress={()=>navigation.push('SignUp')} transparent> 
								<Text white style={{textDecorationLine : 'underline'}}> SignUp with Email </Text>
							</Button>
						</Block>
					</LinearGradient>
